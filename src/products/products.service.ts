...
export class ProductsService {
  ...
  async getProducts(filters: GetProductsQueryDto): Promise<{
    success: boolean;
    message: string;
    data: Product[];
    total: number;
  ...
  async processProducts(products: ContentfulProductItem[]): Promise<{
    total: number;
    created: number;
    updated: number;
    notAffected: number;
  ...
  private hasProductChanged(
    ...
    existing: Product,
  ): { item: UpdateProductDto; isEqual: boolean } {
    ...
    const existingData = _.pick(existing, [
      'name',
      'brand',
      'model',
      'category',
    ...
    return {
      item: contentfulData,
      isEqual: _.isEqual(contentfulData, existingData),
    };
  }
